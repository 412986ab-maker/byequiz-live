...
class TikTokConnector {
  ...
  setupClientListeners() {
    ...
    const userOf = data => data?.user || data || {};
    const userIdOf = data => {
      const user = userOf(data);
      return user.userId || user.uniqueId || data?.userId || data?.uniqueId;
    };
    const uniqueIdOf = data => {
      const user = userOf(data);
      return user.uniqueId || data?.uniqueId;
    };
    const nicknameOf = data => {
      const user = userOf(data);
      return user.nickname || data?.nickname || uniqueIdOf(data);
    };
    const avatarOf = data => {
      const user = userOf(data);
      return user.profilePictureUrl || data?.profilePictureUrl;
    };

    this.client.on('chat', data => {
      this.lastEventTime = Date.now();
      eventBus.dispatch('CHAT', {
        id: userIdOf(data), uniqueId: uniqueIdOf(data),
