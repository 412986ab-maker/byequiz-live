...
class TikTokConnector {
  ...
  async connect(username = this.username) {
    ...
    try {
      // Use the modern connector API. The legacy shim can throw inside
      // getTopViewerAttributes() when TikTok omits optional room-user fields.
      ...
      this.client = new TikTokLiveConnection(this.username, {
  ...
  setupClientListeners() {
    ...
    // TikTok connector versions can expose identity either as data.user
    // or as flat/userInfo fields. Normalize all supported shapes here so
    // chat commands always have a stable participant identity.
    const userOf = data =>
      data?.user || data?.userInfo || data?.userDetails || data || {};
    ...
    const stableUserIdOf = data => {
      return userIdOf(data) || uniqueIdOf(data) || nicknameOf(data);
    };

    this.client.on('chat', data => {
      this.lastEventTime = Date.now();
      eventBus.dispatch('CHAT', {
        id: stableUserIdOf(data), uniqueId: uniqueIdOf(data),
        nickname: nicknameOf(data), avatar: avatarOf(data),
    ...
    this.client.on('gift', data => {
      ...
      eventBus.dispatch('GIFT', {
        ...
        nickname: nicknameOf(data), avatar: avatarOf(data),
        giftId: data?.giftId,
        giftName: giftDetails.giftName || data?.giftName,
        giftCount: data?.repeatCount || data?.giftCount || 1,
        diamondCount: data?.diamondCount || giftDetails.diamondCount || 0,
    ...
    this.client.on('disconnected', () => {
      if (!this.isManualDisconnect && !this.streamEnded) {
        this.handleDisconnectOrError(new Error('انقطع اتصال البث المباشر بشكل مفاجئ'));
      }
    });

    this.client.on('error', payload => {
      const err = payload?.exception instanceof Error
        ? payload.exception
        : new Error(payload?.exception?.message || payload?.info || payload?.message || 'TikTok connector error');
      this.handleDisconnectOrError(err);
    });
  }
