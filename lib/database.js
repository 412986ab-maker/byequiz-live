  ...
  async init() {
    if (process.env.DATABASE_URL) {
      try {
        ...
        // Hydrate the in-memory question cache from PostgreSQL.
        // The server-authoritative game engine uses getQuestionsSync(), so leaving
        // this collection empty makes every NEXT_QUESTION command appear to do nothing.
        try {
          const result = await this.pgPool.query('SELECT * FROM questions ORDER BY created_at ASC');
          if (result.rows.length > 0) {
            this.collections.questions = result.rows.map(r => ({
              ...r,
            ...
            logger.info('Question bank hydrated from PostgreSQL', { count: this.collections.questions.length });
          } else {
            // First production boot: seed the bundled question bank exactly once.
            const qFile = path.join(DATA_DIR, 'questions.json');
            if (fs.existsSync(qFile)) {
              const bundled = JSON.parse(fs.readFileSync(qFile, 'utf8'));
              if (Array.isArray(bundled) && bundled.length > 0) {
                await this.setQuestions(bundled);
                logger.info('Question bank seeded into PostgreSQL', { count: bundled.length });
              }
            }
          }
        } catch (questionErr) {
          logger.error('PostgreSQL question bank hydration failed:', { error: questionErr.message });
        }
        return;
      } catch (err) {
  ...
  async getSettings() {
    if (this.isPostgres && this.pgPool) {
      try {
        ...
        return Object.fromEntries(res.rows.map(r => {
          if (typeof r.value !== 'string') return [r.key, r.value];
          try { return [r.key, JSON.parse(r.value)]; }
          catch (_) { return [r.key, r.value]; }
        }));
